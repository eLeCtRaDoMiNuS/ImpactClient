...
if isfolder("impact") == false then
    makefolder("impact")
end

if isfolder("impact/sound") == false then
    makefolder("impact/sound")
end

if isfolder("impact/sound/mc") == false then
    makefolder("impact/sound/mc")
end

if isfolder("impact/assets") == false then
    makefolder("impact/assets")
end

if isfolder("impact/config") == false then
    makefolder("impact/config")
end

if isfolder("impact/scripts") == false then
    makefolder("impact/scripts")
end

local sliderapi = {}

local foldername = "impact/config"
local conf = {
...
ScreenGuitwo.Name = "ImpactNotificationGui"
...
local function getcustomassetfunc(path)
    if not betterisfile(path) then
        ...
        local req = requestfunc({
            Url = "https://raw.githubusercontent.com/7GrandDadPGN/VapeV4ForRoblox/main/"..path:gsub("impact/assets", "assets"),
            Method = "GET"
        })
...
local function getcustomassetfuncforsounds(path)
    if not betterisfile(path) then
        ...
        local req = requestfunc({
            Url = "https://raw.githubusercontent.com/8pmX8/rektsky4roblox/main/"..path:gsub("impact/sound", "sound"),
            Method = "GET"
        })
        writefile(path, req.Body)
    end
...
local function playdingsound(istrue) 
    if lib["Sounds"] then
        if istrue then
            playsound(getcustomassetfuncforsounds("impact/sound/enable.mp3") or getasset("impact/sound/enable.mp3"))
        else
            playsound(getcustomassetfuncforsounds("impact/sound/disable.mp3") or getasset("impact/sound/disable.mp3"))
        end
    end
end
...
function lib:CreateWindow()
    ...
    function lib:CreateTab(title, color, asset)
        ...
        assetthing.Image = getcustomassetfunc("impact/assets/" .. asset .. ".png") or getasset("impact/assets/" .. asset .. ".png")
        local tabtable = {
            ["Toggles"]={}
        }
            ...
            function sussyamog:CreateDropDown(argstable)
                ...
                dropdownapi["SelectNext"] = function() 
                    ...
                    if newindex then 
                        ...
                        createnotification("NewIndex in selector ("..argstable.Name..") in function `SelectNext` was not found!", "If this keeps happening,o to you exploit's folder\nthen go to workspace/impact/config\nand delete everything inside of that folder", 10, false)
                    end
                end
